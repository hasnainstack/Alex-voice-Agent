...
// How many transcript exchanges before we "reveal" results
const REVEAL_AFTER_EXCHANGES = 3;

type PanelState = "idle" | "searching" | "results";

function usePanelState(status: CallStatus, transcriptCount: number): PanelState {
  ...
  useEffect(() => {
    ...
    if (transcriptCount >= REVEAL_AFTER_EXCHANGES) {
      // Small delay so it feels like results just came back
  ...
  if (status === "active" && !revealed) return "searching";
  return "results";
}
...
export function CompaniesSidebar({ status, transcriptCount }: PanelProps) {
  const panelState = usePanelState(status, transcriptCount);
...
export function CompaniesBottomSheet({ status, transcriptCount }: PanelProps) {
  const [open, setOpen] = useState(false);
  const panelState = usePanelState(status, transcriptCount);

  // Auto-open the sheet when results arrive (only once)
